...
cameraBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        capturedImageFile =
            await window.VKUCamera.takePhoto();

        cameraStatus.textContent =
            "✅ Đã chụp và lưu ảnh vào Gallery";
...
const imageFile =
    capturedImageFile || imageInput.files[0] || null;
const session = {
...
function loadSurveys() {
    ...
    getAllSurveys()
        .then((surveys) => {
            ...
            // survey.html không có surveyList
            // nên không cần hiển thị danh sách tại đây
            if (!surveyList) {
                return;
            }
...
window.addEventListener("online", async () => {
    console.log("🌐 Mạng đã trở lại → bắt đầu đồng bộ...");

    try {
        await syncPendingSurveys();
    } catch (error) {
        console.error("❌ Lỗi đồng bộ khi mạng trở lại:", error);
    }
});